                            ...
                            <form  method="post" enctype="multipart/form-data" class="form-horizontal">
                                <div class="row form-group col-md-12">
                                    <div class="col-sm-12 col-md-4 mt-2">
                                        <input type="date" id="dateInput" name="dateInput" class="form-control-lg form-control" select date>
...
$(document).ready(function() {
    ...
    function checkFormFields() {
        ...
        if (dateInput !== "" && semester !== "0" && course !== "") {
            $('#get_students').prop('disabled', false); // Enable the submit button
        } else {
            $('#get_students').prop('disabled', true); // Keep it disabled
        }
    }

    // When Date input changes, check if all fields are filled
    $("#dateInput").change(function() {
        checkFormFields();
    });

    // When Semester is selected, check if all fields are filled
    ...
    $(document).on("click", "#get_students", function() {
        // Process form, fetch student list and toggle bars as needed
        var selectedDate = $('#dateInput').val();
        var semester = $('#semester').val();
        var course = $('#course').val();
